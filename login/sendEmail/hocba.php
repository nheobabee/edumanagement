...
try {
    ...
    $mail->CharSet = 'UTF-8';
    //Recipients
    $mail->setFrom('vpkcntt@wru.vn', 'Văn phòng Khoa CNTT - Trường ĐH Thủy lợi');

    $mail->addReplyTo('vpkcntt@wru.vn', 'Văn phòng Khoa CNTT - Trường ĐH Thủy lợi');
   
    $row = 1;
    if (($handle = fopen("hocba.csv", "r")) !== FALSE) {
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            echo $row;
            $row++;
            $mail->addAddress($data[15], $data[1]); // Add a recipient
            
            // Attachments
            // $mail->addAttachment('pdf/HB/0'.$data[16].'.pdf', '0'.$data[16].'_1.pdf'); // Add attachments
            $mail->addAttachment('pdf/Giay_bao_mat_sau.pdf', '0'.$data[16].'_2.pdf'); // Optional name
